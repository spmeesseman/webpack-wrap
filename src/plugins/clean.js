...
class WpBuildCleanPlugin extends WpwPlugin
{
	...
	async buildAssets(compilation)
	{
		...
		const distPath = this.app.getDistPath(),
			  compilerOptions = this.app.build.source.config.options.compilerOptions;
	...
	{
		this.compilation = compilation;
		const compilerOptions = this.app.build.source.config.options.compilerOptions,
		...
		if (await existsAsync(this.app.global.cacheDir))
		{
			...
			await Promise.all([
				...
				.map(
					(plugin) => join(
						this.app.global.cacheDir, WpwPlugin.cacheFilename(this.app.mode, plugin)
					)
				)
